...
exports.findAll = (req, res) => {
  Group.findAll({ include: ["user", "users"] }) // TODO: mejorar este join despues
    .then((data) => {
      res.send(data);
    })
    .catch((err) => {
...
exports.findOne = (req, res) => {
  ...
  Group.findByPk(id, { include: ["user", "users"] })
    .then((data) => {
      if (data) {
        res.send(data);
      } else {
...
exports.update = (req, res) => {
  ...
  Group.update(req.body, {
    ...
    .then(async (num) => {
      if (num == 1) {
        const groupUpdated = await Group.findByPk(id, {
          include: ["user", { model: User }],
        });
  ...
  Group.findByPk(groupId)
    .then((group) => {
      if (group) {
        User.findByPk(userId)
          .then((user) => {
            if (user) {
              ...
              res.status(404).send({
                message: "User not found with the provided ID." + userId,
              });
            }
          })
          .catch((err) => {
        ...
        res
          .status(404)
          .send({ message: `Group with ID ${groupId} not found.` });
      }
    })
    .catch((err) => {
      res.status(500).send({
